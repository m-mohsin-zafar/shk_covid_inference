...
def get_all_visits_count(c):
    sql = "SELECT COUNT(DISTINCT (session)) AS visits_count FROM sessions"
    c.execute(sql)
    res = c.fetchall()
    return res


def get_unique_visits_count(c):
    sql = "SELECT COUNT(DISTINCT (ip)) as unique_visits_count FROM sessions"
    c.execute(sql)
    res = c.fetchall()
    return res


def get_home_page_visits_count(c):
    sql = "SELECT SUM(visits) AS home_visits FROM pages WHERE name=='home'"
    c.execute(sql)
    res = c.fetchall()
    return res


def get_prediction_page_visits_count(c):
    sql = "SELECT SUM(visits) AS predictions_made FROM pages WHERE name=='results'"
    c.execute(sql)
    res = c.fetchall()
    return res
